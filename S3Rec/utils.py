...
def get_user_seqs_long_vectorized(data_file):
    rating_df = pd.read_csv(data_file)
    user_seq = rating_df.groupby("user")["item"].agg(list).tolist()
    long_sequence = sum(user_seq, [])
    item_set = set(long_sequence)
    max_item = np.max(list(item_set))  # `set`을 `list`로 변환 후 numpy max 사용
    return user_seq, max_item, long_sequence


def get_item2attribute_json(data_file):
    item2attribute = json.loads(open(data_file).read())
    attribute_set = set()
