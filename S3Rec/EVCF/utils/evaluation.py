...
def evaluate_vae(args, model, train_loader, data_loader, epoch, dir, mode):
    ...
    def Recall_at_k_batch(X_pred, heldout_batch, k=100, first=1):
        batch_users = X_pred.shape[0]
        if first<1:
            print(batch_users, '\n')
        ...
        if first<1:
            print(X_pred_binary[np.arange(batch_users)[:, np.newaxis], idx[:, :k]].shape, '\n')
        X_true_binary = torch.tensor((heldout_batch > 0), dtype=torch.float).cpu()
        tmp = torch.tensor(np.logical_and(X_true_binary, X_pred_binary).cpu(), dtype=torch.float).sum(dim=1)
