...
import pickle
# -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
def load_preprocessed_data(args, **kwargs):
    unique_sid = list()
    dataset = '/opt/ml/input/EVCF/data/train/'
    with open(os.path.join(dataset, "Preprocessed", "unique_sid.txt"), 'r') as f:
        for line in f:
    ...
    x_train = load_train_data(os.path.join(dataset, 'Preprocessed', 'train.csv'))
    all_data = load_train_data(os.path.join(dataset, 'Preprocessed', 'all_data.csv')) ###

    np.random.shuffle(x_train)
    x_val_tr, x_val_te = load_tr_te_data(os.path.join(dataset, 'Preprocessed', 'validation_tr.csv'),
                                         os.path.join(dataset, 'Preprocessed', 'validation_te.csv'))

    x_test_tr, x_test_te = load_tr_te_data(os.path.join(dataset, 'Preprocessed', 'test_tr.csv'),
                                           os.path.join(dataset, 'Preprocessed', 'test_te.csv'))

    # idle y's
    y_train = np.zeros((x_train.shape[0], 1))
    all_data_zeros = np.zeros((all_data.shape[0], 1)) ### 

    # pytorch data loader
    all = data_utils.TensorDataset(torch.from_numpy(all_data), torch.from_numpy(all_data_zeros)) ###
    all_loader = data_utils.DataLoader(all, batch_size=args.test_batch_size, shuffle=False, **kwargs) ###
    ...
    return train_loader, val_loader, test_loader, all_loader, args
...
def load_dataset(args, **kwargs):
    ...
    elif args.dataset_name == 'preprocessed_data':
        train_loader, val_loader, test_loader, all_loader, args = load_preprocessed_data(args, **kwargs)
        return train_loader, val_loader, test_loader, all_loader, args
    else:
